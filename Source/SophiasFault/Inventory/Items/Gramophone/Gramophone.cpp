...
void AGramophone::UseInteraction()
{
	if (_firstShape != nullptr && _secondShape != nullptr && _thirdShape != nullptr) {
		...
		if (firstShapeRot.Roll <= -134.f && firstShapeRot.Roll >= -136.f &&
			secondShapeRot.Roll >= 134.f && secondShapeRot.Roll <= 136.f &&
			thirdShapeRot.Roll <= -44.f && thirdShapeRot.Roll >= -46.f && !_bReadyState) {
			
			_curveFloat->FloatCurve.DeleteKey(_keyHandle);
			_curveFloat->FloatCurve.UpdateOrAddKey(25.f, 1.f);
...
void AGramophone::ControlGramophone()
{
	if (_bReadyState) {
		...
		} else if (_timelineComponent->GetPlaybackPosition() >= 1.8f && _timelineComponent->GetPlaybackPosition() < 21.f) {
			_crank->AddActorLocalRotation(FRotator(-3.f, 0.f, 0.f));
			_record->AddActorLocalRotation(FRotator(0.f, 1.f, 0.f));
		} else if (_timelineComponent->GetPlaybackPosition() >= 22.f) {
